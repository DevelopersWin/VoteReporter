...
{
	public class LoggingConfigurationsTests
	{
		[Fact]
		public void MinimumLevelIs()
		{
			var configuration = new LoggingConfigurations
								{
									new MinimumLevelIsConfiguration { Level = LogEventLevel.Verbose }
								};
		...
		public void MinimumLevelSwitch()
		{
			...
			var configuration = new LoggingConfigurations
								{
									new MinimumLevelSwitchConfiguration { Controller = controller }
								};
			var configured = configuration.Get( new Serilog.LoggerConfiguration() );
			var logger = configured.CreateLogger();
		...
		public void ReadFromKeyValuePairs()
		{
			...
			var configuration = new LoggingConfigurations
								{
									new ReadFromKeyValuePairsConfiguration { Dictionary =  dictionary }
								};
			var configured = configuration.Get( new Serilog.LoggerConfiguration() );
			var logger = configured.CreateLogger();
			Assert.True( logger.IsEnabled( LogEventLevel.Verbose ) );
		...
		public void ReadFromSettings()
		{
			var configuration = new LoggingConfigurations
								{
									new ReadFromSettingsConfiguration { Settings = Settings.Default }
								};
			var configured = configuration.Get( new Serilog.LoggerConfiguration() );
			var logger = configured.CreateLogger();
			Assert.True( logger.IsEnabled( LogEventLevel.Verbose ) );
		...
		public void Filter( Mock<ILogEventFilter> filter )
		{
			var configuration = new LoggingConfigurations
								{
									new FilterConfiguration { Items = { filter.Object } }
								};
			var configured = configuration.Get( new Serilog.LoggerConfiguration() );
			var logger = configured.CreateLogger();
		...
		public void FilterByExcluding( Mock<ISpecification<LogEvent>> specification )
		{
			var configuration = new LoggingConfigurations
								{
									new FilterByExcludingConfiguration { Specification = specification.Object }
								};
			var configured = configuration.Get( new Serilog.LoggerConfiguration() );
			var logger = configured.CreateLogger();
			logger.Information( "Hello World!" );
		...
		public void FilterByIncludingOnly( Mock<ISpecification<LogEvent>> specification )
		{
			var configuration = new LoggingConfigurations
								{
									new FilterByIncludingOnlyConfiguration { Specification = specification.Object }
								};
			var configured = configuration.Get( new Serilog.LoggerConfiguration() );
			var logger = configured.CreateLogger();
			logger.Information( "Hello World!" );
		...
		public void EnrichWithProperty()
		{
			var configuration = new LoggingConfigurations
								{
									new EnrichWithPropertyConfiguration { PropertyName = "EnrichWithProperty", Value = 123, DestructureObjects = false }
								};
			var configured = configuration.Get( new Serilog.LoggerConfiguration() );
			var history = new LoggerHistorySink();
			var logger = configured.WriteTo.Sink( history ).CreateLogger();
		...
		public void Enrich( Mock<ILogEventEnricher> enricher )
		{
			var configuration = new LoggingConfigurations
								{
									new EnrichConfiguration { Items = { enricher.Object } }
								};
			var configured = configuration.Get( new Serilog.LoggerConfiguration() );
			var logger = configured.CreateLogger();
			logger.Information( "Hello World!" );
		...
		public void DestructureType()
		{
			var configuration = new LoggingConfigurations
								{
									new DestructureTypeConfiguration { ScalarType = typeof(Item) }
								};
			var configured = configuration.Get( new Serilog.LoggerConfiguration() );
			var history = new LoggerHistorySink();
			var logger = configured.WriteTo.Sink( history ).CreateLogger();
		...
		public void DestructureMaximumDepth()
		{
			var configuration = new LoggingConfigurations
								{
									new DestructureMaximumDepthConfiguration { MaximumDepth = 67 }
								};
			var configured = configuration.Get( new Serilog.LoggerConfiguration() );
		...
		public void Destructure( Mock<IDestructuringPolicy> policy )
		{
			var configuration = new LoggingConfigurations
								{
									new DestructureConfiguration { Policies = { policy.Object } }
								};
			var configured = configuration.Get( new Serilog.LoggerConfiguration() );
			var logger = configured.CreateLogger();
		...
		public void DestructureByTransform()
		{
			...
			var configuration = new LoggingConfigurations
								{
									new DestructureByTransformConfiguration<Item> { Transform = transform }
								};
			var configured = configuration.Get( new Serilog.LoggerConfiguration() );
			var logger = configured.CreateLogger();
		...
		public void AddTextWriter()
		{
			...
			var configuration = new LoggingConfigurations
								{
									new AddTextWriterConfiguration { Writer = writer, FormatProvider = null }
								};
			var configured = configuration.Get( new Serilog.LoggerConfiguration() );
			var logger = configured.CreateLogger();
