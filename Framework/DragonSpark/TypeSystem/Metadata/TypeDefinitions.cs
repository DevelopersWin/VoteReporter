...
namespace DragonSpark.TypeSystem.Metadata
{
	...
	sealed class TypeDefinitions : FirstSelector<object, TypeInfo>
	{
		...
		{
			public static IParameterizedSource<object, TypeInfo> Implementation { get; } = Sources.Coercion.Extensions.Allow( new TypeInfoDefinitionProvider().Apply( Common<TypeInfo>.Assigned ), Coercer<TypeInfo>.Default );
		...
		{
			public static IParameterizedSource<object, TypeInfo> Implementation { get; } = Sources.Coercion.Extensions.Allow( new MemberInfoDefinitionProvider().Apply( Common<MemberInfo>.Assigned ), Coercer<MemberInfo>.Default );
