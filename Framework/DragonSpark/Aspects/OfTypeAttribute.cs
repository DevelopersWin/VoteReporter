...
namespace DragonSpark.Aspects
{
	public class OfTypeAttribute : LocationContractAttribute, ILocationValidationAspect<Type>
	{
		...
		OfTypeAttribute( ImmutableArray<TypeAdapter> types ) : this( types, $"The specified type is not of type (or cannot be cast to) {string.Join( " or ", types.Select( type => type.Type.FullName ) )}" ) {}
		...
		public Exception ValidateValue( Type value, string locationName, LocationKind locationKind ) => 
			!types.IsAssignableFrom( value ) ? CreateArgumentException( value, locationName, locationKind ) : null;
