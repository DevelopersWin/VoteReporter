...
Summary:
========
A new weekly vote report is out! Featuring vote totals for a 
<xsl:for-each select="local:VoteReport.Groups/local:VoteGroupView"><xsl:if test="position()=last()"><xsl:text>and</xsl:text></xsl:if> <xsl:value-of select="@Title" /> (<xsl:value-of select=".//local:VoteCount/@Count" /> Votes) <xsl:if test="not(position()=last())"><xsl:text>, </xsl:text></xsl:if></xsl:for-each>.
Message:
========
